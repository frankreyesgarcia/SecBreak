...
FOLLOWUP_PATH = DATA_DIR / "rq3_followup.jsonl"
TABLES_PATH = RESULTS_DIR / "rq3_tables.csv"


def gh_headers():
    token = os.environ.get("GITHUB_TOKEN")
    if not token:
        token_path = DATA_DIR.parent / "token.txt"
        if token_path.exists():
            token = token_path.read_text(encoding="utf-8").strip()
    if not token:
        raise SystemExit("ERROR: GITHUB_TOKEN environment variable is required. Set it and re-run.")
    return {
...
def find_followup(repo: str, pr_number: int, merged_at: str):
    ...
    issues = gh_get(f"https://api.github.com/search/issues", params={"q": f"repo:{repo} is:pr created:>={merged_dt.date()} sort:created-asc"})
    earliest = None
    reverted = False
...
def main():
    ...
    plt.title("Time to Follow-up Fix")
    plt.tight_layout()
    ...
    plt.title("Merge Rate by CVSS Severity")
    plt.tight_layout()
